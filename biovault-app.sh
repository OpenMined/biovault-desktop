...
Environment:
  ...
  SEQURE_NATIVE_BIN   Optional syqure binary override (passed through to app instances)
  CODON_PATH          Optional codon path override (passed through to app instances)
...
for i in "${!EMAIL_LIST[@]}"; do
  ...
  env -i \
    ...
    ${SEQURE_NATIVE_BIN:+SEQURE_NATIVE_BIN="$SEQURE_NATIVE_BIN"} \
    ${CODON_PATH:+CODON_PATH="$CODON_PATH"} \
    ${SYQURE_SKIP_BUNDLE:+SYQURE_SKIP_BUNDLE="$SYQURE_SKIP_BUNDLE"} \
    ${SYQURE_DEBUG:+SYQURE_DEBUG="$SYQURE_DEBUG"} \
    ${BV_SYQURE_USE_DOCKER:+BV_SYQURE_USE_DOCKER="$BV_SYQURE_USE_DOCKER"} \
    ${BV_SYQURE_NATIVE_TIMEOUT_S:+BV_SYQURE_NATIVE_TIMEOUT_S="$BV_SYQURE_NATIVE_TIMEOUT_S"} \
    ${BV_SYQURE_PORT_BASE:+BV_SYQURE_PORT_BASE="$BV_SYQURE_PORT_BASE"} \
    ${BV_SYQURE_TRANSPORT:+BV_SYQURE_TRANSPORT="$BV_SYQURE_TRANSPORT"} \
    ${BV_SYQURE_TCP_PROXY:+BV_SYQURE_TCP_PROXY="$BV_SYQURE_TCP_PROXY"} \
    ${BV_SYFTBOX_HOTLINK:+BV_SYFTBOX_HOTLINK="$BV_SYFTBOX_HOTLINK"} \
    ${BV_SYFTBOX_HOTLINK_SOCKET_ONLY:+BV_SYFTBOX_HOTLINK_SOCKET_ONLY="$BV_SYFTBOX_HOTLINK_SOCKET_ONLY"} \
    ${BV_SYFTBOX_HOTLINK_TCP_PROXY:+BV_SYFTBOX_HOTLINK_TCP_PROXY="$BV_SYFTBOX_HOTLINK_TCP_PROXY"} \
    ${BV_SYFTBOX_HOTLINK_QUIC:+BV_SYFTBOX_HOTLINK_QUIC="$BV_SYFTBOX_HOTLINK_QUIC"} \
    ${BV_SYFTBOX_HOTLINK_P2P_ONLY:+BV_SYFTBOX_HOTLINK_P2P_ONLY="$BV_SYFTBOX_HOTLINK_P2P_ONLY"} \
    ${BV_SYFTBOX_HOTLINK_QUIC_ONLY:+BV_SYFTBOX_HOTLINK_QUIC_ONLY="$BV_SYFTBOX_HOTLINK_QUIC_ONLY"} \
    ${SYFTBOX_HOTLINK:+SYFTBOX_HOTLINK="$SYFTBOX_HOTLINK"} \
    ${SYFTBOX_HOTLINK_SOCKET_ONLY:+SYFTBOX_HOTLINK_SOCKET_ONLY="$SYFTBOX_HOTLINK_SOCKET_ONLY"} \
    ${SYFTBOX_HOTLINK_TCP_PROXY:+SYFTBOX_HOTLINK_TCP_PROXY="$SYFTBOX_HOTLINK_TCP_PROXY"} \
    ${SYFTBOX_HOTLINK_QUIC:+SYFTBOX_HOTLINK_QUIC="$SYFTBOX_HOTLINK_QUIC"} \
    ${SYFTBOX_HOTLINK_P2P_ONLY:+SYFTBOX_HOTLINK_P2P_ONLY="$SYFTBOX_HOTLINK_P2P_ONLY"} \
    ${SYFTBOX_HOTLINK_QUIC_ONLY:+SYFTBOX_HOTLINK_QUIC_ONLY="$SYFTBOX_HOTLINK_QUIC_ONLY"} \
